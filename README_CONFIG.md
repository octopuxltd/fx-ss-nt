...
7. **Shared server cache (Netlify):** The AI proxy (`netlify/functions/ai-proxy.js`) caches **successful** upstream responses in **Netlify Blobs**. **Search** and **Firefox** suggestion calls use a **semantic** key (SHA-256 of kind + normalized query from the prompts in `step1.js`), not the full request body: **provider and model do not need to match** for a hit, and a cached **longer** search list can serve a **shorter** request (e.g. 15 strings cached → 10 returned). The proxy **re-synthesizes** a valid OpenAI/OpenRouter/Claude-shaped JSON body for whichever provider the current request uses. Other requests (e.g. warm-up) still use a **legacy** exact `provider + payload` hash key. TTL defaults to **365 days** (`AI_PROXY_CACHE_TTL_MS`, milliseconds—override for shorter freshness). Disable with **`AI_PROXY_CACHE=0`**. Responses include **`X-AI-Proxy-Cache: hit`** or **`miss`** when caching is active. This shares completions derived from the same typed query across users on the same site—tune TTL or turn off if that is not acceptable for your deployment.
